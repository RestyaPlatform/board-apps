        ...
        function insertZapierButton(e) {
            if ($(e.target).hasClass('footer')) {
                ...
                if (sessionStorage.getItem("r_zapier_access_token") !== null) {
                    $('#zapier-modal-description').text('Your Zapier Access token is ' + sessionStorage.getItem("r_zapier_access_token"));
                    $('#js-zapier').text('Close').attr('data-dismiss', 'modal').attr('title', 'Close').removeAttr('id');
                }
            }
        }
        $dc.on('click', '#js-zapier', function(event) {
            event.preventDefault();
            window.open('http://' + window.location.hostname + '/restyaboard/oauth/authorize?response_type=code&client_id=' + r_zapier_client_id + '&state=' + new Date().getTime() + '&redirect_uri=' + window.location.protocol + '//' + window.location.host + window.location.pathname + 'client/apps/r_zapier/login.html', 'DescriptiveWindowName', 'resizable,scrollbars,status');
            return false;
        });
    });
        ...
        if (event.data.app === 'r_zapier') {
            ...
            $.get('oauth_callback/r_zapier/' + code, function(access_token) {
                ...
                if (access_token.indexOf('failed') === -1) {
                    ...
                    $('#zapier-modal-description').text('Your Zapier Access token is ' + zapierToken);
                    $('#js-zapier').text('Close').attr('data-dismiss', 'modal').attr('title', 'Close').removeAttr('id');
                } else {
