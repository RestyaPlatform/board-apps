                ...
                if (board_response.lists) {
                    $.each(board_response.lists, function(list_key, list) {
                        if (list.custom_fields) {
                            ...
                            if (customFieldArr.completed_cards && customFieldArr.completed_cards == 'Yes') {
                                if ($('.js-board-list').closest("[data-list_id='" + list.id + "']").length) {
                                    if ($('.js-board-list').closest("[data-list_id='" + list.id + "']").find('.js-completed-card-icon').length === 0) {
                                        var elem = $('<span class="pull-left js-completed-card-icon"><i class="icon-check text-danger"></i><span>');
                                        $('.js-board-list').closest("[data-list_id='" + list.id + "']").find('.js-show-edit-list-form').prepend(elem);
                                    }
                                }
                            }
        ...
        if ($(e.target).parents('div.js-workflow-cards-form').length) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (formData.completed_cards && formData.completed_cards == 'Yes') {
                        if ($('.js-board-list').closest("[data-list_id='" + list_id + "']").find('.js-completed-card-icon').length === 0) {
                            var elem = $('<span class="pull-left js-completed-card-icon"><i class="icon-check text-danger"></i><span>');
                            $(e.target).parents('.js-list-head').find('.js-show-edit-list-form').prepend(elem);
                        }
                    } else {
                        $('.js-board-list').closest("[data-list_id='" + list_id + "']").find('.js-completed-card-icon').remove();
                    }
                }
            });
        }
