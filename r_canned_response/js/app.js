(function() {
    ...
    $dc.ready(function() {
        ...
        $('body').append('<div class="modal fade" id="r_canned_response_modal" tabindex="-1" role="dialog" aria-labelledby="cannedResponseModalLabel" aria-hidden="false"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal"><span aria-hidden="true" id="js-cammed-resonse-close">×</span><span class="sr-only">Close</span></button><div class="media list-group-item-heading"><div class="pull-left"><img class="img-circle" width="36" height="36" src="' + local_storage_apps.r_canned_response.icon + '" /></div><div class="media-body"><h4 class="modal-title" id="exampleModalLabel">' + i18next.t('Canned Response') + '</h4><div><span class="text-muted">v' + local_storage_apps.r_canned_response.version + '</span> By <a target="_blank" href="' + local_storage_apps.r_canned_response.author_url + '?utm_source=restyaboard&utm_medium=apppopup&utm_campaign=rb-app-canned-response-v0.1.1' + '" title="author">' + local_storage_apps.r_canned_response.author + '</a></div></div></div></div><div class="modal-body import-block"><ul><li>' + i18next.t('Adds a canned response dropdown next to comment box in card view. Using this you can quickly insert canned response') + '.</li><li>' + i18next.t('You can add your own canned response in ') + '<code>' + i18next.t('    client/apps/r_canned_response/json/app.json') + '</code>.</li></ul></div><div class="modal-footer"><a id="js-canned-response-button" href="#" title="' + i18next.t('Close') + '" class="btn btn-primary">' + i18next.t('Close') + '</a></div></div></div></div>');
    });
